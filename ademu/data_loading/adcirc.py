...
from ..constants import KAT_EX_PATH, NEW_ORLEANS, DATA_PATH
from ..preprocessing.sel import trim_tri
        ...
        ds = xr.Dataset(
            data_vars=dict(
                U10=(
                    ["time", "lat", "lon"],
                    data[:, 0, :, :],
                    {"units": "m s**-1", "long_name": "Zonal 10m windspeed"},
                ),
                V10=(
                    ["time", "lat", "lon"],
                    data[:, 1, :, :],
                    {"units": "m s**-1", "long_name": "Meridional 10m windspeed"},
                ),
            ),
            coords=dict(
...
def main():
    # python ademu/data_loading/adcirc.py
