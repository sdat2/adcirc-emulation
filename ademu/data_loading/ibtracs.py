...
from ..constants import IBTRACS_NC, GOM_BBOX, NO_BBOX, LANDING_DS
from ..conversions import fcor_from_lat, si_ify, knots_to_ms
...
def reduce_to_req(ds: xr.Dataset) -> xr.Dataset:
    ...
    Example::
        >>> from ademu.data_loading.ibtracs import katrina, reduce_to_req, REQ_VAR
...
def filter_by_labels(
    ...
    Example of using it to filter for North Atlantic TCs::
        >>> import xarray as xr
        >>> from ademu.constants import IBTRACS_NC
        >>> from ademu.data_loading.ibtracs import filter_by_labels
...
def _track_in_bbox(lons: np.ndarray, lats: np.ndarray, bbox: List[float]) -> bool:
    ...
    Example of tracking:
        >>> from ademu.data_loading.ibtracs import _track_in_bbox
...
def _point_in_bbox(lon: float, lat: float, bbox: List[float]) -> bool:
    ...
    Example of tracking:
        >>> from ademu.data_loading.ibtracs import _point_in_bbox
...
def time_steps(input: xr.Dataset) -> xr.Dataset:
    ...
    Example:
        >>> from ademu.data_loading.ibtracs import katrina, time_steps, na_tcs
...
if __name__ == "__main__":
    # python ademu/data_loading/ibtracs.py
    # print(na_tcs())
    # print(gom_tcs())
    # print(katrina())
