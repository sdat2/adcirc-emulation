...
from .constants import UREG, RADIUS_EARTH
...
def knots_to_ms(knots_input: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    Example::
        >>> from ademu.conversions import knots_to_ms
        >>> knots_to_ms(1)
        0.5144444444444445
    """
...
def nmile_to_meter(nmile_input: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    Example::
        >>> from ademu.conversions import nmile_to_meter
        >>> nmile_to_meter(1.0)
        1852.0
    """
...
def meter_to_nmile(meter_input: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    Example::
        >>> from ademu.conversions import meter_to_nmile
...
def distance_between_points(pt1: Point, pt2: Point) -> float:
    ...
    Examples::
        >>> from sithom.place import Point
        >>> from ademu.conversions import distance_between_points
        >>> point1 = Point(0, 0)
        >>> point2 = Point(10, 0)
...
def millibar_to_pascal(
    ...
    Example::
        >>> from ademu.conversions import millibar_to_pascal
...
def pascal_to_millibar(
    ...
    Example::
        >>> from ademu.conversions import pascal_to_millibar
...
def kelvin_to_celsius(
    ...
    Example::
        >>> from ademu.conversions import kelvin_to_celsius
...
def celsius_to_kelvin(
    ...
    Example::
        >>> from ademu.conversions import celsius_to_kelvin
...
def fcor_from_lat(lat: Union[float, np.ndarray]) -> Union[float, np.ndarray]:
    ...
    Example::
        >>> from ademu.conversions import fcor_from_lat
        >>> fcor_from_lat(90) / 2
...
def si_ify(input: xr.Dataset) -> xr.Dataset:
    ...
    Example::
        >>> import numpy as np
        >>> import xarray as xr
        >>> from ademu.conversions import si_ify
